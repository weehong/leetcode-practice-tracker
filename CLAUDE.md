...
- **LeetCode**:
  - Requires `LEETCODE_SESSION_ID` cookie for fetching questions
  - **IMPORTANT**: Premium accounts also require `LEETCODE_CSRF_TOKEN` for proper authentication
  - Both values should be set in `.env` file or provided via interactive prompts
  - The application sends both cookies and the `x-csrftoken` header to LeetCode's GraphQL API
