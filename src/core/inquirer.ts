...
class Inquirer {
  ...
  promptCsrfToken = async () =>
    await inquirer.prompt({
      type: "password",
      name: "csrfToken",
      message:
        "Please enter your LeetCode CSRF Token (optional, recommended for Premium users):",
    });
  ...
  promptFetchContent = async () =>
    await inquirer.prompt({
      type: "confirm",
      name: "fetchContent",
      message: "Do you want to fetch detailed question content (descriptions, examples, constraints)?",
      default: true,
    });
}
