...
class Terminal {
  ...
  questionMenu = async (answer: string) => {
    switch (answer) {
      ...
      case "fetch-favorite-question":
        ...
        const favoriteQuestions = await this._fetchFavoriteQuestionHandler(sessionResp2.session, sessionResp2.csrfToken);
      ...
      case "fetch-grind-question":
        try {
          ...
          const { fetchContent } = await this._inquirier.promptFetchContent();

          this._grind = new Grind(weeks, hours, "weeks", difficulty);

          // Fetch questions with or without detailed content
          let grindQuestionWeek;
          if (fetchContent && this._leetcode) {
            console.log('\n🔍 Fetching detailed question content from LeetCode...');
            console.log('⏱️  This may take a few minutes depending on the number of questions...\n');

            const startTime = Date.now();
            grindQuestionWeek = await this._grind.getQuestionsWithContent(this._leetcode);
            const endTime = Date.now();
            const duration = Math.round((endTime - startTime) / 1000);

            console.log(`⏱️  Content fetching completed in ${duration} seconds\n`);
          } else {
            grindQuestionWeek = await this._grind.getQuestions();
            if (fetchContent && !this._leetcode) {
              console.log('\n⚠️  LeetCode service not available. Fetching basic questions only.');
              console.log('To get detailed content, please configure LEETCODE_SESSION_ID in your .env file.');
            }
          }
  ...
  private async setupLeetCodeSession(): Promise<{ session: string; csrfToken?: string }> {
    const config = ConfigManager.getConfig();
    ...
    if (config.leetcode.sessionId && config.leetcode.sessionId.trim()) {
      logger.info('Using LeetCode session ID from environment configuration', {
        hasCsrfToken: !!config.leetcode.csrfToken
      });
      return {
        session: config.leetcode.sessionId,
    ...
    // Prompt for CSRF token (optional but recommended for Premium users)
    const csrfResp = await this._inquirier.promptCsrfToken();

    return {
      session: sessionResp.session,
      csrfToken: csrfResp.csrfToken && csrfResp.csrfToken.trim() ? csrfResp.csrfToken : undefined
    };
  }
  ...
  private _leetCodeQuestionHandler = async (session: string, query: string, csrfToken?: string): Promise<any> => {
    this._leetcode.setSessionId(session, csrfToken);
    this._spinner.start();
  ...
  private _leetCodeFavoriteQuestionHandler = async (session: string, csrfToken?: string): Promise<any> => {
    this._leetcode.setSessionId(session, csrfToken);
    this._spinner.start();
  ...
  private async _fetchLeetCodeQuestionHandler(
    sessionResp: { session: string; csrfToken?: string } & { [x: string]: {} },
    query: string
  ): Promise<Question[]> {
    ...
    const questionResp: any[] = await Promise.all([
      ...
      await this._leetCodeFavoriteQuestionHandler(sessionResp.session, sessionResp.csrfToken).catch((error) => {
  ...
  private async _fetchFavoriteQuestionHandler(session: string, csrfToken?: string): Promise<Question[]> {
    this._spinner.text = "Fetching favorite questions from LeetCode";

    // First, get the favorite lists
    const favoriteLists = await this._leetCodeFavoriteQuestionHandler(session, csrfToken);
